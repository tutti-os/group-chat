...
export function AgentAvatar(props: {
  ...
  return (
    ...
    >
      <RoomAvatar title={props.title} avatar={centerAvatar} provider={centerAvatar ? null : props.provider} size={size} contentSizeOffset={contentSizeOffset} className={"[relative] [z-index:0]"} />
      {props.hideProviderBadge ? null : (
        <span
          className={"[position:absolute] [top:-2px] [right:-2px] [z-index:1] [display:grid] [place-items:center] [border-radius:999px] [color:var(--text-secondary)] [background:var(--white-stationary)] [box-shadow:0_0_0_1px_var(--border-1)]"}
