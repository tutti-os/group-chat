import { BrainCircuit, Bot, LoaderCircle, X } from "lucide-react";
  ...
  return (
    <div className={"[display:flex] [flex-wrap:wrap] [gap:6px] [padding:0_clamp(14px,_2.25vw,_32px)_8px] [background:var(--panel)] max-[1080px]:[padding-inline:16px] max-[760px]:[padding:0_12px_8px]"} aria-label="临时任务栏">
      ...
      {props.agentRuns.map((run) => (
        <div
          key={run.id}
          className={`[display:inline-flex] [max-width:min(280px,_100%)] [align-items:center] [gap:2px] [border:1px_solid_var(--border)] [border-radius:999px] [padding:2px_2px_2px_4px] [color:var(--text)] [background:#ffffff] [font-size:12px] [font-weight:650] ${props.openAgentRunId === run.id ? "[border-color:var(--primary)] [background:var(--accent-soft)]" : ""}`}
        >
          <button
            type="button"
          ...
          >
            ...
            <Bot size={13} />
            <span className={"[min-width:0] [overflow:hidden] [text-overflow:ellipsis] [white-space:nowrap]"}>{run.participantName} 执行中</span>
          </button>
          <button
            type="button"
            className={"[display:grid] [flex:0_0_auto] [width:22px] [height:22px] [place-items:center] [border:0] [border-radius:999px] [color:var(--muted)] [background:transparent] [cursor:pointer] hover:[color:var(--text)] hover:[background:#00000008]"}
